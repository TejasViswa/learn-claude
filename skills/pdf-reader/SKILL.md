...
All scripts use a venv at `~/.claude/skills/pdf-reader/.venv` with `pymupdf` installed. If the venv is missing, create it from `requirements.txt`:

```bash
python3 -m venv ~/.claude/skills/pdf-reader/.venv
~/.claude/skills/pdf-reader/.venv/bin/pip install -r ~/.claude/skills/pdf-reader/requirements.txt
```

**Python command:** Always invoke scripts with:
```
~/.claude/skills/pdf-reader/.venv/bin/python ~/.claude/skills/pdf-reader/scripts/<script>.py [args]
```

## Scripts

All scripts are in `~/.claude/skills/pdf-reader/scripts/`.
